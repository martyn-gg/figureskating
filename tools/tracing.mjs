...
import { lobeSense, label, exitState, lobeContinues, TURNS } from '../src/lib/skating.js';
...
console.log('tracing direction (the picture against the rule, all 40 combinations)\n');
...
for (const s of STATES) for (const turnKey of Object.keys(TURNS)) {
  const t = TURNS[turnKey];
  const x = exitState(s, turnKey);
  const { frames, turnIdx } = buildTrace({ entry: s, turn: turnKey });
  const name = `${label(s)} ${turnKey}`;

  const inSense = iceSense(frames, 0, turnIdx);
  const outSense = iceSense(frames, turnIdx, frames.length - 1);
